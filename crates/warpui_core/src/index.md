...
| `ChromeSettings.swift` | the window chrome's numbers — sidebar width, panel margin, glass opacity — with the range each is clamped to, and their tolerant decoding |
| `SettingsStore.swift` | the versioned settings document and the `UserDefaults` it is kept in |
...
`ChromeSettings` and `SettingsStore` are the two files in this folder that are **not** view-layer files, and
the split is deliberate rather than tidiness: they are `Foundation`-only, so the harnesses compile them and a
harness can hold the ranges, the clamping, and what happens to a settings file written by another build.
`Theme.swift` beside them imports AppKit and is not compiled by the harnesses. That is why
`Scripts/run-tests.sh` names both files individually instead of globbing the directory — a glob would let a UI
